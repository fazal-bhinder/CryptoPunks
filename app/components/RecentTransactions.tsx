...
const RecentTransactions = ({
  ...
  const next = () =>
    setCurrentTransactionIndex(
      (p) => (p + 1) % Math.max(1, transactionData.length - 2),
    );
  const prev = () =>
    setCurrentTransactionIndex(
      (p) =>
        (p - 1 + Math.max(1, transactionData.length - 2)) %
        Math.max(1, transactionData.length - 2),
    );
    ...
    >
      ...
      <div className="relative overflow-hidden">
        <motion.div
          ...
          animate={{ x: -currentTransactionIndex * 200 }}
