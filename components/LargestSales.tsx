...
const LargestSales = ({ salesData, currentSaleIndex, setCurrentSaleIndex }: LargestSalesProps) => {
  const next = () =>
    setCurrentSaleIndex((p) => (p + 1) % Math.max(1, salesData.length - 2));
  const prev = () =>
    setCurrentSaleIndex((p) => (p - 1 + Math.max(1, salesData.length - 2)) % Math.max(1, salesData.length - 2));
  ...
  return (
    <motion.section initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="py-16 max-w-6xl mx-auto px-6">
      ...
      <div className="relative overflow-hidden">
        <motion.div className="flex space-x-6" animate={{ x: -currentSaleIndex * 200 }} transition={{ type: 'spring', stiffness: 300, damping: 30 }}>
          {salesData.map((s, i) => <SaleCard key={i} {...s} />)}
        </motion.div>
